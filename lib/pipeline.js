...
let imageLayerOff = false; // /api/image 返回 501（未配 IMAGE_MODEL）后整段会话内不再尝试
...
function parseJSON(text) {
  // 剥思考过程：思考类模型会输出 <think>…</think>（或只剩残缺闭标签），
  // 思考里若含示例JSON会污染候选提取，先整体剥掉
  let s0 = String(text).replace(/<think>[\s\S]*?<\/think>/gi, '');
  const ct = s0.search(/<\/think\s*>/i);
  if (ct !== -1) s0 = s0.slice(s0.indexOf('>', ct) + 1);
  const cleaned = s0.replace(/```json|```/gi, '').trim();
...
async function chatJSON(messages, opts) {
  ...
  for (let i = 0; i < 2; i++) {
    try {
      const msgs = i === 0 ? messages : [...messages, { role: 'user', content: '刚才的输出无法解析。重新输出：只输出 JSON 本体，第一个字符必须是 {，不要思考过程、不要解释、不要代码围栏。' }];
      lastRaw = await chat(msgs, opts);
...
// 字段级兜底：JSON 解析成功但关键字段为空（典型于输出被 max_tokens 截断后强行修复），
// 带强化指令自动重试一次，把"一阵一阵的生产中断"挡在门内
async function chatJSONRequire(messages, opts, key, hint) {
  let data = await chatJSON(messages, opts);
  let v = data && data[key];
  if (Array.isArray(v) ? v.length : v) return data;
  const msgs = [...messages, { role: 'user', content: `刚才的 JSON 缺少有效的 ${key} 字段。${hint || ''}重新完整输出整个 JSON，只输出 JSON 本体，第一个字符必须是 {。` }];
  data = await chatJSON(msgs, opts);
  return data;
}
 ...
 */
export async function runProduction({ profile, hot, onLog, onTopics, onNote, shouldStop }) {
  ...
  const topicData = await chatJSONRequire(topicPrompt(profile, hot), { temperature: 0.85 }, 'topics', '必须包含 topics 数组（5个选题）。');
  ...
  // ─ 单篇生产（抽出便于重试）─
  async function produceNote(topic, idx, tag, feed) {
    log(`${tag}「${topic.title || topic.angle || '今日选题'}」开写：先出 10 个候选标题 + 正文 + 封面文案…`);
    const draft = await chatJSONRequire(notePrompt(profile, topic, feed), { temperature: 0.9, max_tokens: 6000 }, 'titles', '必须包含 titles 数组（至少8个标题对象）。');
    ...
    // ─ 封面底图：优先用老板投喂的真实照片；其次才走 AI 生图 ─
    let bgDataUrl = '';
    if (feed && feed.coverDataUrl && !feed.useAIImage) {
      bgDataUrl = feed.coverDataUrl;
      log(`${tag} 封面底图采用老板提供的真实照片 ✓`, 'ok');
    }
    const bgPrompt = (draft.cover && draft.cover.bgPrompt) || '';
    if (!bgDataUrl && bgPrompt && !imageLayerOff) {
      log(`${tag} 封面底图生成中：「${String(bgPrompt).slice(0, 28)}…」`);
      try {
        ...
        if (r.status === 501) {
          imageLayerOff = true;
          log(`${tag} 生图层未启用（未配置 IMAGE_MODEL），全程使用纯色版式`);
        } else if (!r.ok || d.error) {
          log(`${tag} 底图生成失败（${d.error || 'HTTP ' + r.status}），本篇降级纯色版式`, 'warn');
        } else if (d.dataUrl) {
          bgDataUrl = d.dataUrl;
          log(`${tag} 底图就位，封面切换图底大字报版式 ✓`, 'ok');
        }
      } catch (e) {
        log(`${tag} 底图生成异常（${e.message}），本篇降级纯色版式`, 'warn');
      }
    }

    log(`${tag} 封面已按「${bgDataUrl ? '图底大字报' : (draft.cover?.template || 'V01')}」版式排版，自检通过 ✓`, 'ok');
    ...
    return {
      ...
      cover: { ...(draft.cover || { template: 'V01', title: (top3[0].text || '').slice(0, 9) }), bgDataUrl },
      tip: draft.tip || '',
    };
  }
  ...
  for (let idx = 0; idx < picked.length; idx++) {
    if (shouldStop && shouldStop()) {
      log(`收到叫停指令，已停在第 ${idx} 篇之后。已完成的 ${notes.length} 套笔记保留在交付区。`, 'warn');
      break;
    }
...
/**
 * 投喂模式：基于老板单篇投喂的素材（主题/结构/文风/照片），精做一篇。
 * 与日更不同：① 主题由 feed 指定，不自主选题；② 封面优先用真实照片。
 */
export async function runSingleNote({ profile, feed, onLog, onNote, shouldStop }) {
  const log = (text, type = 'info') => onLog && onLog({ time: now(), text, type });

  log('收到本篇投喂任务，阿桃开始按要求创作…');
  log(`主题：「${feed.theme}」 · 文风：${feed.tone}${feed.structureKey && feed.structureKey !== 'auto' ? ' · 结构：' + feed.structure : ''}`);
  if (feed.photos && feed.photos.length) log(`已收到 ${feed.photos.length} 张真实照片${feed.coverDataUrl && !feed.useAIImage ? '，其中 1 张将作封面底图' : ''}`);

  if (shouldStop && shouldStop()) { log('已收到叫停指令，未开始生产。', 'warn'); return { notes: [] }; }

  // 用 feed 主题构造一个 topic（不走选题引擎）
  const topic = {
    title: feed.theme,
    angle: feed.structureKey && feed.structureKey !== 'auto' ? feed.structure : '紧扣主题自然展开',
    keyword: feed.theme,
    type: '老板投喂',
  };

  const tag = '本篇';
  log(`${tag} 开写：先出 10 个候选标题 + 正文 + 封面文案…`);

  let draft;
  let lastErr;
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      if (attempt > 1) { log(`${tag} 第 2 次尝试（已等待 4 秒）…`, 'warn'); await sleep(4000); }
      draft = await chatJSONRequire(notePrompt(profile, topic, feed), { temperature: 0.9, max_tokens: 6000 }, 'titles', '必须包含 titles 数组（至少8个标题对象）。');
      break;
    } catch (e) { lastErr = e; log(`${tag} 生产失败（第 ${attempt} 次）：${e.message}`, 'warn'); }
  }
  if (!draft) throw new Error(`本篇生产失败：${lastErr?.message || '未知'}`);

  const rawTitles = (draft.titles || [])
    .map((t) => (typeof t === 'string' ? { text: t, formula: '' } : t))
    .filter((t) => t && t.text).slice(0, 10);
  if (rawTitles.length === 0) throw new Error('未生成有效标题');
  ...
  const judgeMap = {};
  if (Array.isArray(judged)) for (const j of judged) if (j && typeof j === 'object' && Number.isInteger(j.i)) judgeMap[j.i] = j;
  const scored = rawTitles.map((t, i) => {
    const j = judgeMap[i] || {};
  ...
  // 封面：优先真实照片
  let bgDataUrl = '';
  if (feed.coverDataUrl && !feed.useAIImage) {
    bgDataUrl = feed.coverDataUrl;
    log(`${tag} 封面底图采用老板提供的真实照片 ✓`, 'ok');
  } else {
    const bgPrompt = (draft.cover && draft.cover.bgPrompt) || '';
    if (bgPrompt) {
      log(`${tag} 无指定封面照片，AI 配图中：「${String(bgPrompt).slice(0, 28)}…」`);
      try {
        const r = await fetch('/api/image', {
        ...
        const d = await r.json().catch(() => ({}));
        if (r.status === 501) log(`${tag} 生图层未启用，使用纯色版式`);
        else if (!r.ok || d.error) log(`${tag} AI 配图失败（${d.error || 'HTTP ' + r.status}），降级纯色版式`, 'warn');
        else if (d.dataUrl) { bgDataUrl = d.dataUrl; log(`${tag} AI 底图就位 ✓`, 'ok'); }
      } catch (e) { log(`${tag} AI 配图异常（${e.message}），降级纯色版式`, 'warn'); }
    }
  }

  log(`${tag} 封面已排版，自检通过 ✓`, 'ok');
